...
public class FindingMatching {
    ...
    public static void main(String[] args) {
        ...
        Optional<Dish> dish = DishData.getAll().stream().filter(Dish::isVegetarian).findAny(); //it returns any vegetarian dish if there is any vegetarian. Optional<> will learn later
        System.out.println(dish.get()); // get method to return the object otherwise it will return Optional

        System.out.println("****FIND FIRST****");
        Optional<Dish> dish2 = DishData.getAll().stream().filter(Dish::isVegetarian).findFirst(); //It returns the first match
        ...
        List<String> list1 = Arrays.asList("Jhonny", "David", "Jack", "Duke", "Jill", "Danny", "Julia", "Jenish", "Divya");
        List<String> list2 = Arrays.asList("Jhonny", "David", "Jack", "Duke", "Jill", "Danny", "Julia", "Jenish", "Divya");
